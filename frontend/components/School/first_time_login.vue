  ...
  <UModal v-model:open="open" :close="false" :dismissible="false" class="max-w-screen-lg">
    <template #body>
      <form class="w-full" @submit.prevent="firstTimeLoginFunc()">
        <UAlert v-if="authStore.authUser?.is_first_time_login" color="info" icon="i-lucide-terminal"
          title=" Please fill up all the required fields" />
        <UAlert v-else color="warning"
          ...
          icon="meteor-icons:bullhorn" title="NOTICE" variant="solid" />
              ...
              <main class="space-y-2 col-span-3">
                ...
                <div class="grid grid-cols-2 gap-5 ">
                  <UFormField :error="error?.password && error?.password[0]" label="Password" name="password" required>
                    <UInput v-model="firstTimeLoginCredential.password" :type="show ? 'text' : 'password'"
                      class="w-full" placeholder="Password" size="lg" variant="outline">
                      <template #trailing>
                        <UButton :aria-label="show ? 'Hide password' : 'Show password'" :aria-pressed="show"
          ...
          <template #school>
            <Placeholder class="aspect-video">
              <main class="space-y-2 col-span-3">
                ...
                <div class="grid grid-cols-2 gap-5 ">
                  <UFormField :error="error?.sec_permit && error?.sec_permit[0]" label="SEC Registration Permit "
                    required>
                  ...
                  <UFormField hint="Optional" label="SEC Registration Permit Expiration">
                ...
                <div class="grid grid-cols-2 gap-5 ">
                  <UFormField :error="error?.shs_provisional_permit && error?.shs_provisional_permit[0]"
                    label="SHS Provisional Permit" required>
                    <UInput class="w-full" size="md" type="file" variant="outline"
                      accept="application/pdf,application/vnd.ms-excel"
                ...
                <div class="grid grid-cols-2 gap-5 ">
                  <UFormField :error="error?.mayors_permit && error?.mayors_permit[0]" label="Mayor's Permit" required>
                    <UInput class="w-full" size="md" type="file" variant="outline"
                      accept="application/pdf,application/vnd.ms-excel"
                  ...
                  <UFormField hint="Optional" label="Mayor's Permit Expiration">
                ...
                <div class="p-10">
                  <table class="w-full  text-xs text-left rtl:text-right text-gray-500">
                    ...
                    <tbody>
                      ...
                      <tr v-for="(track, index) in firstTimeLoginCredential.program_offered" :key="track.id"
        ...
        <div class="flex gap-2 justify-between mt-10">
          ...
          <UButton color="neutral" leading-icon="material-symbols:logout-rounded" @click="authStore.logout('School')">
            Logout
          </UButton>
