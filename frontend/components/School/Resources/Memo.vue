          ...
          <tbody>
            ...
            <tr v-for="announcement in data?.data" v-show="!isLoading" :key="announcement.id"
              ...
              <td class="px-6 py-4 space-x-2">
                ...
                {{ moment(announcement.created_at).format('MMMM D YYYY, h:mm:ss A') }}
              </td>


              <td class="px-6 py-4 space-x-2">

              <QadAnnouncementQadPreviewAnnouncement :announcement_id="announcement.id"/>
              </td>
            </tr>
...
import { downloadAttachment, fetchMemo } from '~/shared/API/School/ResourcesApi';
...
const { data, isLoading } = useQuery({
  queryKey: ['SCHOOL_RESOURCES_MEMO', page, direction],
  queryFn: () => fetchMemo(page, search.value, sort, direction),
})
watch(() => search.value, debounce(() => {
  queryClient.invalidateQueries({ queryKey: ["SCHOOL_RESOURCES_MEMO"] });
}, 300))
