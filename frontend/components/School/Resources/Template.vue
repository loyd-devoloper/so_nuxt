            ...
            <tr v-for="template in data?.data" v-show="!isLoading" :key="template.id"
              ...
              <td class="px-6 py-4 space-x-2">
                ...
                <UButton label="Download" size="sm" icon="material-symbols:download" color="info" variant="outline"
                  @click="downloadTemplateFunc({ id: template.id, filename: template.name })" />
              </td>
            </tr>
...
import { fetchTemplate } from '~/shared/API/School/ResourcesApi';
import { downloadTemplate } from '~/shared/API/Qad/TemplateApi';
...
const { data, isLoading } = useQuery({
  queryKey: ['SCHOOL_RESOURCES_TEMPLATE', page, direction],
  queryFn: () => fetchTemplate(page, search.value, sort, direction),
})
watch(() => search.value, debounce(() => {
  queryClient.invalidateQueries({ queryKey: ["SCHOOL_RESOURCES_TEMPLATE"] });
}, 300))
