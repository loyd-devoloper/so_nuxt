...
import type {SchoolCredentialType} from "~/shared/types/LoginType";
const authStore = useAuthStore();
const show = ref(false)

const credentials = reactive<SchoolCredentialType>({
...
const {error,mutate:loginFunc,isPending} = useMutation({
  mutationFn: () => authStore.schoolLogin(credentials),
  onSuccess: (data) => {
    console.log(data)
    if(data.status === 201)
    {
